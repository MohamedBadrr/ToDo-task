...
showform(false);
  ...
  return (
    <div className='form-container'>
      <form onSubmit={handleSubmit} className="flex flex-col space-y-4 add-task-form">
        <div className='task-title'>
        <h4>{language === 'en' ? 'New Task' : ' مهمة جديدة'}</h4>
        <span><i class="fa-solid fa-xmark" onClick={()=>{showform(false)}}></i></span>
        </div>
        <p>{language === 'en' ? 'Add now task from here' : 'اضف  مهمة جديدة من هنا '}</p>
      <input
        type="text"
